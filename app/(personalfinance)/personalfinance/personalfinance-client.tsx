...
export default function PersonalFinancePage() {
  ...
  return (
    <div className="container mx-auto p-4 space-y-2">
      <h1 className="text-2xl font-bold text-sky-500">Personal Finance - Home Upkeep Expenses</h1>
      <p className="text-gray-300">
        Track and manage your home-related expenses. All data is stored locally in your browser.
      </p>
      <Accordion
        type="single"
        collapsible
        className="w-full bg-amber-500 rounded-lg border border-black"
        defaultValue="house-upkeep"
      >
        <AccordionItem value="house-upkeep">
          <AccordionTrigger className="text-xl px-2">Home Upkeep Expenses:</AccordionTrigger>

          <AccordionContent>
            <Card>
              {/* Optional: keep a header inside the collapsed area */}
              <CardHeader>
                <CardTitle>Home Upkeep</CardTitle>
              </CardHeader>

              <CardContent>
                {/* INNER ACCORDION: your existing section groups */}
                <Accordion type="multiple" defaultValue={["home", "hoa"]} className="mt-2 ">
                  {[...SECTIONS, ...PLACEHOLDERS].map((section) => (
                    <AccordionItem key={section.id} value={section.id}>
                      <AccordionTrigger className="text-lg">{section.title}</AccordionTrigger>
                      <AccordionContent>
                        {section.fields.length === 0 ? (
                          <div className="text-sm text-muted-foreground">Placeholder for future fields.</div>
                        ) : (
...
function SummaryBox({ label, amount }: { label: string; amount: number }) {
  return (
    <div className="rounded-2xl border p-4 bg-gray-500 shadow-md">
